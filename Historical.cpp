...
void Historical::loadData () {
	...
	for ( int i = 0; i < size; i++ ) {
		data.setArrayIndex(i);
		Transaction transaction(data.value(QString::number(TransactionData::debit)).toBool(),
		                        data.value(QString::number(TransactionData::payerName)).toString(),
		                        data.value(QString::number(TransactionData::payerIBAN)).toString(),
		                        data.value(QString::number(TransactionData::receiverName)).toString(),
		                        data.value(QString::number(TransactionData::receiverIBAN)).toString(),
		                        data.value(QString::number(TransactionData::amount)).toFloat(),
		                        data.value(QString::number(TransactionData::causal)).toString(),
		                        data.value(QString::number(TransactionData::date)).toDate());
		rawHistory.push_back(transaction);
	}
...
void Historical::updateOrder () {
	...
	if ( researchOptions.getOrderOptions() == OrderOptions::all ) {
		for ( auto iterator = orderedHistory.begin(); iterator != orderedHistory.end(); iterator++ )
			if ( iterator->getDate() < researchOptions.getDateFrom() ||
			     iterator->getDate() > researchOptions.getDateTo())
				iterators.push_back(iterator);
			else if ( !(iterator->getPayerIBAN().contains(researchOptions.getSearchText(), Qt::CaseInsensitive) ||
			            iterator->getReceiverIBAN().contains(researchOptions.getSearchText(), Qt::CaseInsensitive) ||
			            iterator->getPayerName().contains(researchOptions.getSearchText(), Qt::CaseInsensitive) ||
			            iterator->getReceiverName().contains(researchOptions.getSearchText(), Qt::CaseInsensitive) ||
			            iterator->getCausal().contains(researchOptions.getSearchText(), Qt::CaseInsensitive)))
				iterators.push_back(iterator);
			else {
				if ( iterator->isDebit())
