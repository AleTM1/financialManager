...
void InvestmentManager::loadData () {
	...
	for ( int i = 0; i < size; i++ ) {
		...
		if ( data.value(QString::number(InvestmentData::investmentType)).toString() == "Azione" ) {
			...
			dynamic_cast<Stock *>(investment)->setSharesNumber(
					data.value(QString::number(InvestmentData::stockSharesNum)).toFloat());
		} else if ( data.value(QString::number(InvestmentData::investmentType)).toString() == "Obbligazione" ) {
			investment = new Bond;
			dynamic_cast<Bond *>(investment)->setMonthsDuration(
					data.value(QString::number(InvestmentData::bondMonthsNumber)).toInt(), false);
			dynamic_cast<Bond *>(investment)->setDeadlineDate(
					data.value(QString::number(InvestmentData::bondDeadline)).toDate());
		}
