...
void View::updateInvestmentManager() {
  ...
  for (auto &investment : investmentManager->getInvestmentList()) {
    ...
    if (auto stock = dynamic_cast<Stock *>(investment)) {
      ...
      investmentForm->ui_investmentForm->label_stockChangeAmount->setText(
          QString::number(stock->getSharesNumber()*company->getShareCost() - investment->getTotalInvested()) + "€");
      ...
      investmentForm->ui_investmentForm->label_deadlineDate->setText(bond->getDeadlineDate().toString(
          "dddd, dd / MMMM / yyyy"));
      investmentForm->ui_investmentForm->label_bondCoupon->setText(QString::number(company->getMontlyCoupon()));
      investmentForm->ui_investmentForm->label_bondExpectedYield->setText(
          QString::number((company->getMontlyCoupon())/100*investment->getTotalInvested()*bond->getMonthsDuration())
              + "€");
    }
    investmentForm->show();
...
void View::searchHistorical() {
  controller->changeHistoricalOrder(viewWindow->radioButton_cronologicalOrder->isChecked(),
                                    ...
                                    viewWindow->dateEdit_from->date(),
                                    viewWindow->dateEdit_to->date(),
...
void View::doInvestment() {
  if (viewWindow->comboBox_investmentType->currentText()=="Azione") {
    controller->doInvestment(InvestmentType::stock,
                             viewWindow->label_enetityISIN->text(),
                             viewWindow->lineEdit_stockshareNumber->text().toFloat());
  } else if (viewWindow->comboBox_investmentType->currentText()=="Obbligazione") {
    controller->doInvestment(InvestmentType::bond,
                             viewWindow->label_enetityISIN->text(),
...
void View::updateInvestmentAmount() {
  ...
  if (viewWindow->comboBox_investmentType->currentText()=="Azione") {
    viewWindow->label_totalInvestment->setText(QString::number(viewWindow->lineEdit_stockshareNumber->text().toFloat()
    ...
    viewWindow->label_expectedYield->setText(QString::number(viewWindow->lineEdit_investmentAmount->text().toFloat()
                                                                 *(viewWindow->comboBox_monthsNumber->currentText().toInt())
                                                                 *(((viewWindow->label_coupon->text()).remove('%').toFloat())
                                                                     /100)));
  }
