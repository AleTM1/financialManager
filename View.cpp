...
void View::updateHistorical () {
	...
	for ( auto &transaction : historical->getHistory()) {
		...
		transactionForm->transactionForm_ui->label_date->setText(
				transaction.getDate().toString("dddd, dd / MMMM / yyyy"));
...
void View::updateInvestmentManager () {
	...
	for ( auto &investment : investmentManager->getInvestmentList()) {
		...
		investmentForm->ui_investmentForm->label_buyDate->setText(
				investment->getBuyDate().toString("dddd, dd / MMMM / yyyy"));
		investmentForm->ui_investmentForm->label_totalInvestment->setText(
				QString::number(investment->getTotalInvested()) + "€");
		...
		if ( auto stock = dynamic_cast<Stock *>(investment)) {
			...
			investmentForm->ui_investmentForm->label_stockShareNumber->setText(
					QString::number(stock->getSharesNumber()));
			investmentForm->ui_investmentForm->label_stockActualInvestment->setText(
					QString::number(stock->getSharesNumber() * company->getShareCost()) + "€");
			investmentForm->ui_investmentForm->label_stockShareCost->setText(
					QString::number(company->getShareCost()) + "€");
			investmentForm->ui_investmentForm->label_stockChangeAmount->setText(QString::number(
					stock->getSharesNumber() * company->getShareCost() - investment->getTotalInvested()) + "€");
			...
			if ( investment->getTotalInvested() < stock->getSharesNumber() * company->getShareCost()) {
				investmentForm->ui_investmentForm->label_stockActualInvestment->setStyleSheet(
						"QLabel { color : green; }");
			...
			investmentForm->ui_investmentForm->label_bondMonthsNumber->setText(
					QString::number(bond->getMonthsDuration()));
			investmentForm->ui_investmentForm->label_deadlineDate->setText(
					bond->getDeadlineDate().toString("dddd, dd / MMMM / yyyy"));
			investmentForm->ui_investmentForm->label_bondCoupon->setText(QString::number(company->getMontlyCoupon()));
			investmentForm->ui_investmentForm->label_bondExpectedYield->setText(QString::number(
					(company->getMontlyCoupon()) / 100 * investment->getTotalInvested() *
					bond->getMonthsDuration()) + "€");
		}
...
void View::searchHistorical () {
	...
	controller->changeHistoricalOrder(viewWindow->radioButton_cronologicalOrder->isChecked(),
	                                  viewWindow->lineEdit_searchHistorical->text(),
	                                  viewWindow->dateEdit_from->date(), viewWindow->dateEdit_to->date(),
	                                  viewWindow->comboBox_options->currentText());
...
void View::doInvestment () {
	...
	if ( viewWindow->comboBox_investmentType->currentText() == "Azione" )
		controller->doInvestment(InvestmentType::stock, viewWindow->label_enetityISIN->text(),
		                         viewWindow->lineEdit_stockshareNumber->text().toFloat());
	else if ( viewWindow->comboBox_investmentType->currentText() == "Obbligazione" )
		controller->doInvestment(InvestmentType::bond, viewWindow->label_enetityISIN->text(),
...
void View::updateInvestmentAmount () {
	...
	if ( viewWindow->comboBox_investmentType->currentText() == "Azione" ) {
		viewWindow->label_totalInvestment->setText(QString::number(
				viewWindow->lineEdit_stockshareNumber->text().toFloat() *
				(viewWindow->label_stockCost->text().remove("€").toFloat())));
	} else if ( viewWindow->comboBox_investmentType->currentText() == "Obbligazione" ) {
	...
	if ( viewWindow->label_totalInvestment->text().toFloat() >
	     dynamic_cast<Conto *>(model->accessDataStorage("Conto"))->getLiquid()) {
		viewWindow->label_totalInvestment->setStyleSheet("QLabel { color : red; }");
		viewWindow->label_totalInvestment->setText(
				viewWindow->label_totalInvestment->text() + "€ (liquidità insufficiente)");
	} else {
