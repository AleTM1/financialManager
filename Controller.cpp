...
void
Controller::changeHistoricalOrder ( bool cronologicalOrder, QString txt, QDate from, QDate to, QString optionOrder ) {
...
void Controller::doTransaction ( bool d, std::map<TransactionData, QString> dataTransaction, QDate date ) {
	...
	Transaction transaction(d, dataTransaction[payerName], dataTransaction[payerIBAN], dataTransaction[receiverName],
	                        dataTransaction[receiverIBAN], (dataTransaction[amount]).toFloat(), dataTransaction[causal],
	                        date);
	...
	switch ( model->doTransaction(transaction)) {
		case 0:
			...
			break;
		case 1:
			model->makeMessageDialogNoButtons("transazione fallita", "Nome dell'ordinante non valido.");
			break;
		case 2:
			model->makeMessageDialogNoButtons("transazione fallita", "IBAN dell'ordinante non valido.");
			break;
		case 3:
			model->makeMessageDialogNoButtons("transazione fallita", "Nome del beneficiario non valido.");
			break;
		case 4:
			model->makeMessageDialogNoButtons("transazione fallita", "IBAN del beneficiario non valido.");
			break;
		case 5:
			model->makeMessageDialogNoButtons("transazione fallita", "Importo non valido.");
			break;
		case 6:
		...
		default:
			model->makeMessageDialogNoButtons("transazione fallita", "Errore sconosciuto");
	}
...
void Controller::accountSave ( std::map<AccountData, QString> strings ) {
	...
	switch ( model->saveAccount(strings)) {
		case 0:
			...
			model->makeMessageDialogNoButtons("salvataggio riuscito",
			                                  "Aggiornamento dei dati effettuato con successo!");
			break;
		case 1:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Nome troppo breve.");
			break;
		case 2:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Cognome troppo breve");
			break;
		case 3:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Il Codice Fiscale deve avere 16 caratteri.");
			break;
		case 4:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Città non valida.");
			break;
		case 5:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Il CAP deve avere 5 cifre.");
			break;
		case 6:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Indirizzo troppo breve.");
			break;
		case 7:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Il numero di telefono deve avere 10 cifre.");
			break;
		case 8:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Indirizzo mail non valido.");
			break;

		default:
			model->makeMessageDialogNoButtons("salvataggio fallito", "Errore sconosciuto");
	}
...
void
Controller::doInvestment ( InvestmentType investmentType, QString ISINcode, float investmentAmount, int monthsNumber ) {
